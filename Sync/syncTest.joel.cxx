...
class vtkRenderWindowListener : public vtkCommand
{
	public:
		vtkRenderWindow *window;

		static vtkRenderWindowListener *New(vtkRenderWindow *newWindow)
		{
			vtkRenderWindowListener *vtkRendWindList = new vtkRenderWindowListener;
			vtkRendWindList->window = newWindow;
			return vtkRendWindList;
		}
		
		virtual void Execute(vtkObject *window, unsigned long, void*)
		{
			
			vtkCommand *com = reinterpret_cast<vtkCommand*>(window);
			
			vtkRenderWindowListener *callerWindow = static_cast<vtkRenderWindowListener*>(com);
			vtkRenderWindow* vtkrw = vtkRenderWindow::New();
			vtkrw = callerWindow->window;
			vtkrw->Render();
...
int main()
{
  ...
  coneMapper2->SetInputConnection( cone->GetOutputPort() );
  ...
  ren2->SetBackground( 0.0, 0.5, 1.0 );
  ...
  sharedCamera->ShallowCopy(ren1->GetActiveCamera());

  vtkRenderWindowListener *renWinL =   vtkRenderWindowListener::New(renWin);
  vtkRenderWindowListener *renWinL2 =   vtkRenderWindowListener::New(renWin2);

  ren1->SetActiveCamera(sharedCamera);
  ren2->SetActiveCamera(sharedCamera);

  //sharedCamera->AddObserver(vtkCommand::ModifiedEvent, renWinL);
  //sharedCamera->AddObserver(vtkCommand::ModifiedEvent, renWinL2);
